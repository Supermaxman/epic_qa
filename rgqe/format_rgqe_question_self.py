...
def load_predictions(input_path):
	...
	for file_name in os.listdir(input_path):
		if file_name.endswith('.question_self'):
			preds = torch.load(os.path.join(input_path, file_name))
			pred_list.extend(preds)
	sample_entail_pairs = defaultdict(lambda: defaultdict(dict))
	probs = []
	for prediction in tqdm(pred_list):
		...
		sample_entail_pairs[question_id][answer_a_id][entailed_set_id] = entail_prob
