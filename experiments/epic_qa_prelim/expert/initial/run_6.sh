...
export RUN_NAME=HLTRI_RERANK_6
export SEARCH_RUN=passage-1200-expanded
...
python search/search_pass_index.py \
  --index_path indices/expert/expanded_pass \
...
python rerank/epic_eval.py \
  data/prelim_judgments.json \
  models/${MODEL_NAME}/${RUN_NAME}.txt \
  rerank/.${DATASET}_ideal_ranking_scores.old.tsv \
  --task ${DATASET}